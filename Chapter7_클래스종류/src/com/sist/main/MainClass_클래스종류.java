 ...
 *        			*** 인스턴스의 초기화는 생성자가 더 편하고
 *        				static의 초기화는 static 초기화블록이 더 편하다
 *        				static은 생성자를 호출할 필요가 없기때문에 ***
 *        			-------------------------------------------
 *        			클래스내에서 선언되는 내용 : 멤버변수, 멤버메소드
 *        			*** 클래스 {}안에서는 구현할 수 없고 선언만 가능하다
 *        				int a = 10; // 이건 가능
 *        				int a;
 *        				a = 10; // 이건 불가능 (구현) -> 연산자, 메소드 호출, 제어문 --> {} : 초기화블록, 메소드{}, 생성자{}
 *        			*** 초기화블록은 자동처리, 상속예외 조건
 *     2. 클래스
 *        1) 클래스 설계
 *        2) 클래스 메모리에 저장 --> 저장하는 과정 (인스턴스화) --> 저장된 상태 (인스턴스, 객체)
 *        	 클래스명 객체명 = new 클래스명() --> new 연산자는 메모리가 생성
 *        	 new 생성자()
 *        	 new 생성자() 다른 메모리가 만들어짐
 *        3) 활용 : 
 *        	 객체.변수, 객체.메소드() ....
 *        4) 소멸
 *        	 객체 = null; (주소값을 지운다) --> GC
 ...
 *   -----------------------------------------------------------------------------
 *   객체지향의 특성을 만든다 (유지보수가 쉽게 제작 -> 권장사항 문법사항X)
 *     = 캡슐화
 *     	  데이터보호와 동시에 필요시에는 사용이 가능하게 만든다 (변수)
 *     	  -> 변수 (읽기/쓰기) --> getter/setter
 *     = 상속/포함 : 재사용
 *        상속 : 변경이 필요한 경우 -> 게시판을 다양한 형태로 바꿀때
 *        포함 : 변경없이 사용	 -> 게시판기능에 댓글기능 추가시킬때
 *        상속이 안되는경우 -> 초기화블록, 생성자, static, private
 *        				 형식)
 *        					  class A extends B
 *        						    -		  -
 *        				   		 하위클래스		상위클래스
 ...
 *     					상위 클래스 : this
 *     					하위 클래스 : this, super
 *         super() => 상속 내린 클래스이 생성자 호출 (생성자 안에서 사용 => 첫줄)
 *         
 *         예) 
 *             public class Parent {
 *             		public Parent(int a) {}
 ...
 *      public interface A : 추상 클래스의 일종 => interface 추상 클래스의 단점 보완
 *      										 |		-------- 단일상속
 *      									  --------- 다중상속가능
 ...
 *     * 메소드중에 abstract가 있는 경우에 private을 사용하면 안된다
 *     * 메소드중에 private, final을 동시에 사용할 수 없다
 *     
 *     375page : 추상클래스 - 녹색 C 우상단에 A
 *     			 - 자신이 메모리 할당을 할 수 없다, 미완성된 클래스 -> 사용시는 구현된 클래스의 주소를 받아서 사용
 *     		=> class A
 *     		   class B extends A
 *     			
 *     		   B b = new B(); --> 불가능
 ...
 *     -------- 기능이 여러가지인 경우에 설계만 해둔다 (프로그램에 맞게 구현해서 사용 => 구현 안됨)
 *     			예) 버튼, 마우스, 키보드
 *     			** 메모리 할당이 불가능하다 => 구현한 클래스를 대입해서 사용한다
 *     			** 데이터베이스의 모든 클래스는 추상 클래스로 제작되어있다
 *     			   --------					   --------
 *     				 | 종류가많다 (오라클, MySQL, MSSQL, MaridDB, DB2...)
 *     						   ------------		   --------
 *     형식)
...
abstract class 도형 {
	...
	/*
	 *    추상 클래스는
	 *    	= 클래스마다 따로 구현해야되는 메소드 (선언)
	 *      = 필요시마다 구현해야되는 메소드 (구현 => 오버라이딩)
	 *      = 멤버 변수도 가지고있다
	 *      = 사용시에는 반드시 메소드를 구현한 클래스를 이용해서 메모리할당을 한다
