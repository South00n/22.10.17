 ...
 *     		 - 선택 조건문 : 조건이 true, false일 경우 따로 만들어서 처리
 ...
 *     
 *     	  3) 반복문
 *     		 - for문 : 반복횟수가 지정된 경우 (자바에서 많이씀)
 *     		 - while문 : 반복횟수가 없는 경우 (무한루프) --> 선조건 (데이터베이스에서 많이 씀)
 *     		 - do-while문 : 한번이상은 반드시 수행해야 되는 경우 --> 후조건
 *     
 *        4) 반복제어문
 *        	 - break : 반복 중지
 ...
 *       	if(조건)
 *       	   수행문장 1
 *       	   수행문장 2 ---> 무조건 수행하는 문장 (if문 소속문장이 아니기때문)
 *       	   ================= 조건 false면 수행문장 2
 *       						 조건 true면 수행문장 1, 수행문장 2
 *       	   **자바에서 지원하는 모든 제어문은 바로 밑에 문장 1개만 제어 할 수 있다**
 *       	   **만약에 여러개의 문장을 if문에서 제어할 때는 {} 을 사용**
 *       
 *       	if(조건) {
 *       	   수행문장 1
 *       	   수행문장 2
