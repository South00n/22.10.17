 ...
 *              = 단일 상속을 기본으로 한다
 *              = 객체 생성하는 방법
 *                하위클래스 a = new 하위클래스() ==> 기본 형식 (상위클래스를 확장 -> 따라서 상위클래스를 불러올필요가 없음) => 상속받은 클래스를 주로 이용한다
 *                => 상위클래스 a = new 상위클래스() ==> 거의 사용빈도가 없다
 *                상위클래스 a = new 하위클래스() : 인터페이스, 추상클래스, 스프링, 데이터베이스
 *                		     -------------   ---------------- 객체 생성을 할 수 없다
 *                							 (미완성 클래스 => 사용자가 완성해서 사용해야한다)
 *                							 *** 인터페이스 (스프링의 기반) : 기술면접 잘 나옴
 *                *** 객체생성시 => 생성자명이 다르다 (인터페이스, 패턴)
 *                Calendar c = Calendar.getInstance();
 *                Date d = new Date(); 일반 클래스
 *                List list = new ArrayList(); (List 미완성 ArrayList 완성)
 *                Map map = new HashMap();
 *                ---------------------------- 객체 생성할떄 생성자와 클래스명은 항상 같지않다.
 ...
 *         	 }
 *         	 class B extends A {
 *         	   public void display(int a) {} // 오버로딩
