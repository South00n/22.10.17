 ...
 *					멤버변수 (인스턴스 변수) 
 * 					---------------------------
 * 					메소드 (구현이 안된 메소드) ==> 공통적으로 적용 (구현 내용이 다를 경우)
 * 					public abstract 리턴형 메소드(매개변수..);
 * 					---------------------------
 * 					메소드 (구현이 된 메소드) ==> 선택적으로 적용
 * 					---------------------------
 * 				}
 *       *** 3) 인터페이스 : 추상 클래스의 일종 => 추상 클래스의 단점을 보완한 클래스 (다중 상속이 가능) 모든 프로그램에서 중심이되는 클래스
 ...
 *        
 *        		=> 다중 상속이 가능하다 (interface도 클래스다) ==> 서로다른 클래스를 연결해서 사용
 *        												 ---------------- 리모콘
