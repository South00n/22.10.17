 ...
 *     		class ClassName {
 *     			---------------------------
 *     			 속성, 	핃드  -->  멤버변수 (캡슐화) --> getter/setter (변수 : 읽기/쓰기) = 인스턴스 변수
 *     			 ** 공유하는 변수 (static)
 *     			 ** 변수는 기본형만 있는것이 아니다 (배열, 클래스도 가능)
 *     			---------------------------
 ...
 *     	  = this 키워드 : 객체 자신을 나타내주는 키워드
 *     		--------- this는 모든 클래스에 사용
 *     				  ---- 생성자, 인스턴스메소드 (static메소드는 this가 없다)
 *     				  this의 특징 -> static Object this;
 *     				  this 생성되는 시점 -> 객체 생성시에 저장 --> 생성된 객체의 주소값
 *     				  A a = new A();
 *     				  -> 컴파일러 (this = a)
 *     				  A b = new A();
 *     				  -> 컴파일러 (this = b)
 *     				  this는 자신의 객체 전송, 지역변수와 구분
 *     2) 상속 --> 객체지향의 특성 (데이터보호 방법, 재사용, 수정, 추가, 공통적인 내용을 모아서 모듈화)
 *     						  -------	   ----	 ---  ---				   ----
 ...
 *     				 Sub s = new Sub();
 *     					--- 사용 가능한것
 *     						s.a, s.b, s.aaa(), s.bbb(), s.c, s.ccc()
 *     			  3) 상위 클래스로 생성 받기는 하위클래스로 받는다
 *     				 Sub s = (Sub)new Super();
 *     				 
 *     				 
 ...
 *        	  인터페이스는 관련된 여러개의 클래스를 묶어서 관리하는 목적 (메소드가 동일)
 *        	  => 공통점
 ...
 *        			}
 *        			------------------------------ 추상클래스가 사라진다
 *        			아직 추상클래스를 사용하는 라이브러리가 존재
 *        			------------------------------ 추상클래스가 나온다면 -> 여러개 기능이있다 
 *     4) 예외처리
 *        = 예외처리 종류 (예외복구, 예외회피)
 ...
 *       StringTokenizer, Dater, Calendar, SimpleDateFormat
