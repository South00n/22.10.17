 ...
 *    2. 메소드
 *    	 리턴형 / 매개변수
 *    	 ----   ----- 사용자 전송, 상세보기(번호 : 영화, 게시물) C/S => 클라이언트 (유저 : 요청 => 매개변수_
 *    	 화면출력 (배열, 클래스)							   서버 => 결과값 (리턴형)
 *    			---	 ---- 상세보기
 *    			목록출력
 *    	 기본형 => 로그인 여부, 갯수, 총페이지
 *    
 ...
 *    	-------------
 *    	 출력할 내용이 있는 경우 : 목록 -> 배열
 *    	 Recipe[] recipeListData() 레시피 데이터출력
 *    							상세 = 클래스
 *    	 Recipe recipeDetailDate(int no) no번에있는 레시피 데이터를 출력
 *    							총페이지 = 기본형
 *    	 int recipeTotalPage()
 *    						 	로그인 = 기본형
 ...
 *    	오버로딩
 *    	------
 *    	 같은 메소드명으로 새로운 기능을 추가할때 사용하는 기술
 *    	 조건
 *    	 ---
 *    	  같은 클래스내
 *    	  같은 메소드명
