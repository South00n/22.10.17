 ...
 * 		  1) 인스턴스
 * 		     리턴형 메소드명(매개변수..) {
 * 				return 값; --> 생략이 가능 --> 결과값 (없는경우 -> void -> return생략이 가능)
 * 			 }
 * 		  2) 정적 (static)
 * 			 static 리턴형 메소드명(매개변수..) {
 * 				return 값;
 * 			 }
 * 	   2. 메소드 호출 방식 -------> 메소드도 동일 // 메소드는 메모리 저장을 해서 사용
 * 		  1) 인스턴스 : new로 저장
 * 			 A a = new A();
 * 			 class A {
 * 				int a;
 ...
 * 				--------------------------  저장이 완료
 * 			 }
 * 			 클래스명.a , 클래스명.list() ---- 클래스 변수, 클래스 메소드
 * 
 * 			 라이브러리 --> 클래스명.메소드()
 * 						 Math.random(), String.vlaueOf()
...
public class 메소드_1 {
	...
	void swap(String name1, String name2) { // 오버로딩 : 같은 클래스에서 같은 메소드를 이용해서 새로운 기능 첨부
		String temp = name1;
		name1 = name2;
		name2 = temp;
	...
	public static void main(String[] args) {
		...
		// x, y 메모리 ==> 메소드 호출 (매개변수 다른 메모리가 생성) => 복사본 (원본은 그대로 복사본안에서만 변경)
