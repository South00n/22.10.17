 ...
 *       	   long a = 10;
 *       				-- int
 *       	   int a = 100000000000; 오류
 *       long : 빅데이터, 금융권, 증권, 학교 ...
 *       			   ----- 통계, 회계 (ERP) --> 관리프로그램 (데이터베이스/ 오라클과 자바 연동하는 과정)
 *       		(8byte = 64bit)
 *       정수변수를 설정 --> int
 *    ------------------------------------------------------
 ...
 *    ===============================
 *    
 *  --> 변수설정, 명칭 .. (20page ~ 21page)
 *     식별자 (변수, 메소드, 클래스, 인터페이스)
 *       - 변수, 메소드는 소문자로 시작 (문법x 약속o) --> 두개이상의 단어 ( _사용 or 두번째단어 대문자)
 *         (firstname --> first_name or firstName) , 들여쓰기
 *       - 클래스, 인터페이스 대문자로 시작
 *       -------------------------------------------------------------------------
 *       1) 알파벳, 한글로 시작을 한다 (단 한글은 운영체제에 따라 다르다 --> 가급적이면 알파벳을 사용한다)
 *       	AWS (리눅스(우분투) 한글깨짐)
 *       2) 숫자를 사용할 수 있다 (단 앞에 사용금지)
 *       3) 특수문자 사용이 가능 ( _ , $ ) -> _ (두개단어 결합, 임시변수( _ , temp -> 문자를 자를떄, 변수값 교환))
 *		 4) 키워드 사용 금지
 *			---- 자바에서 이미 사용중인 단어 
 *			-->int, double ..
 * 			   int int, int double
 ...
 * 		데이터형 변수명; --> 선언
 * 		변수명 = 값 --> 초기화
 * 		데이터형 변수명 = 값 --> 선언과 동시에 초기화 (명시적 초기화)
 * 
 * 		같은 데이터형이 여러개 있는 경우는 , 사용가능
 * 		int a,b,c,d,e;
 ...
 *  	double d = 10.6 ...
 * 		-----------------------------------------------
 * 		큰 데이터형에는 작은 데이터값을 대입해도 가능 (자동 형변환)
 * 		int a = 'A' --> a = 65
