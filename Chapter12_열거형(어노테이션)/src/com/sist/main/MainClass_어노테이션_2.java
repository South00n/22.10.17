 ...
 *    => @FunctionallInterface => 람다식
 *    => 스프링, Mybatis, JPA도 이미 제작 되어있따 => 인덱스
 *    -------------------------------------------------- 703
 *    711page ==> 사용자 정의
 *    	 ***** 찾는 내용은 항상 어노테이션 밑에 있거나 옆에 있다
 *    1. Target : 구분자
 *    	 클래스 찾기 => Target(TYPE) ==> type (클래스와 관련) => 클래스혀으 인터페이스, 열거형
 *    	 class ClassName {
 *    		---------------------
 *    		변수     Target(FIELD)
 *    		=> 일반변수는 사용하지 않는다
	...
	public static void main(String[] args) {
		...
		try {
			...
			for(Method m : methods) {
				RequestMapping re = m.getAnnotation(RequestMapping.class);
				// Method위에 있는 메소드 찾기
				if(re.value().equals(cmd)) {
