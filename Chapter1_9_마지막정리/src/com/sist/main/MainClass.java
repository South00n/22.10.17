 ...
 * 		     2) 멤버변수 (인스턴스 변수)
 * 			 3) 공유변수 (클래스 변수 : static) => static 메모리공간이 1개만 생긴다
 *       같은타입 저장 : 배열 => 1차 배열
 *       **다른타입 여러개 : 클래스
 *         = 클래스
 *         	 1) 구성 요소
 ...
 *         				 예매 내역 출력
 *         		---------------------------------------
 *         		 변수 초기화 : 생성자 (id => id 읽기, 예매번호 => 자동 처리) ==> 컴포넌트
 *         		---------------------------------------
 *         		 컴포넌트(클래스 => 한개의 전체기능을 가지고 있다) ==> 조립하면 웹사이트 완성
 *         		 ==> 객체 지향의 특성을 첨부 (유지보수를 편하게 만든다 => 문법 사항은 아니고 권장사항)
 *         			 캡슐화 : 데이터 보호 => 데이터를 은닉화, 메소드를 통해서 변수 사용
 *         							   -----------	-----------------
 *         							   private			 읽기/쓰기 => getter/setter
 *         			 상속/포함 : 재사용 (소스 중복 제거)
 *         			 다향성 : 재사용 필요시에는 언제든지 수정, 기능 추가
 *         									 (오버라이딩), (오버로딩)
 *         		 ==> 클래스의 종류
 *         			 = 추상 클래스
 *         			 **= 인터페이스 : 독립적으로 사용이 가능, 서로 다른 클래스를 연결해서 사용 (리모콘)
 *         			     결합성이 낮은 프로그램이 가능
 *         				 ---- 클래스의 영향 (클래스를 수정시에 다른 클래스에 영향) ==> 스프링 [결합성 낮다]
 *         				 응집성
 *         				 ---- 메소드관련 (한가지 기능) ==> 재사용 가능
 ...
 *         							  class B extends A
 *         							  -----------------
 *         							  B b = new B(); // 확정된 클래스 사용
 *         							  A a = new B();
 *         							  A a = new A(); // 오류 추상클래스는 객체생성 불가
 *         
 *         							  interface A
 *         							  interface B
 ...
 *         				
 *       **원하는만큼 저장 : 컬렉션 (11장 ~ 12장) : **List, Set(중복된 데이터를 제거), **Map(코딩테스트) => 클래스관리
 *       									------ 데이터베이스				 => 웹 지원하는 모든 클래스
 *       																  Request, Response, Session, Cookie
 *       	** 12장 (어노테이션) => 스프링은 전체가 어노테이션
 *       -------------- 메모리 저장
 *       파일 (15장) => 파일 입출력 : File / FileReader / FileWriter / FileInputStream / FileOutputStream
 *       			  ObjectInputString / ObjectOutputStream / BufferedReader / BufferedWriter
 ...
 * 					   +,-,*,/,%  ==, !=, <, >, <=, >=, &&, ||, =, +=, -=
 * 					 = 삼항연산자 => if ~ else로 변경가능 , 소스를 줄이는 경우에 주로 사용
 * 						 (조건)? 값:값
 * 					 = instanceof, equals
 * 					   객체 비교		문자열 비교	
 * 
 ...
 * 			
 * 						  => void외에는 반드 return을 사용한다 (retrun => 메소드 종료)
 * 						  => 리턴형 : 요청처리에 대한 결과값
 ...
 * 		-----------------
 * 		오라클
 * 		  String name, sex, addr, tel, psot, content
 * 		  int age
 * 
