 ...
 *    	  4) 재사용기법
 *    	  -----------------------------------------------
 *    	  단점) 실행 속도가 느리다, 소스 코딩을 이해하기 어렵다 (오버라이딩 : 기존의 메소드의 기능을 변경)
 *    		   단일 상속이다
 *    	  5) 상속의 사용법 (eXtends : 확장)
 *    		 class A extends B
 *    			  ---		--- 상위클래스 : B클래스를 확장해서 새로운 클래스를 만든다
 *    			 하위클래스
 *    			 데이터형은 상속을 내리는 클래스가 크다
 *    			 메모리공간은 상속을 받는 클래스가 크다
 *    		 예)
 *    			 class A {
 *    				int a, b;
 *    				void display() {}
 ...
 *    
 *     			 상속 형식
 *     			 class 하위클래스 extends 상위클래스 (1개만 받을수있음) {
 *     				기능 => 공통적인 내용이 있을때 사용
 *     			 }
 ...
 * 			  ----	   -------
 * 	  Sub : a, b, c, print()
 * 			----  |	 ------- 인스턴스 ==> 따로 메모리 생성 (메모리만 복제)
 * 				 --- static ==> 공통으로 사용
 * 	  복제 ==> 상위 클래스가 먼저 메모리 할당 ==> 하위 클래스가 메모리 할당
 * 	  상속받은 메소드 변경해서 사용 ==> 오버라이딩
 */
