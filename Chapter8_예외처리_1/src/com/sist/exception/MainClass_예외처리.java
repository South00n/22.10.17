 ...
 * 		----------------------------------------------
 * 		견고한 프로그램 : 에러가 발생시 종료하지 않고 정상수행이 가능하게 만드는것
 ...
 * 			   500 => 문법에러
 * 			   415 => 한글변환 (UTF-8) -> UFT-8로 잘못기입
 * 			   400 => 전송 데이터 --> 받는 데이터의 데이터형이 다른 경우
 * 					  "홍길동" ---> int로 받으면
 * 			   ---------------------------------------------------------------
 * 		 	   자바 => Exception : 주소값이없다 - NullPointException
 * 								  배열의범위 초과 - ArrayIndexOutOfBoundsException
 * 					  --> output창
 * 		3) 예외처리의 종류
 * 		   ***= 예외 복구 : 예외가 발생하면 복구해서 다시 수행이 가능하게 만드는것 **가장많이사용
 * 				try {
 * 					정상 수행문장
 * 					=> 에러발생할 수 있다
 * 				} catch() {
 * 					에러가 발생시 복구하는 위치
 * 				}
 * 		   ***= 예외 회피 : 에러부분을 제외하고 다음 문장으로 넘어가서 수행
 ...
 * 		   *목적 : 프로그램의 비정상 종료를 방지하고 정상 상태를 유지를 위함
 *      5) 자바에서 지원하는 예외처리의 계층구조
 *         Error : 메모리가 부족, 윈도우가 작동을 안한다, 이클립스문제 발생
 *         Exception : 파일명이 틀리다, IP가 틀리다, 웹 사이트주소가 틀리다..., SQL문장을 잘못 수행
 *      			   Object
 *      				 |
 ...
 *         		= 지금까지 코딩한 부분
 *         	  } catch (예외처리 종류) {
 *         	 	에러발생시 처리하는 영역 -> 대부분은 에러확인용
 *         		예상되는 예외처리 ==> catch(여러번 사용가능)
 *            } finally {
 ...
 *            1. 고민 : 예상되는 에러
 *            예) 
 *            	 두개의 정수를 문자열로 받아서 정수배열에 저장 / 저장된 데이터를 나누기를 하는 프로그램을 제작
 *            	 					   ----- 배열 인덱스           ----	
 *            						   문자열 -> 정수
 *            	// 예외처리는 생각한거 + 1 (기타)
 *            2. 고민 : 예외처리의 위치
