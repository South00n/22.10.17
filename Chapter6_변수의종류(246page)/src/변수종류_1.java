 ...
 *    											}
 *    	-----------------------------------------------------
 *    	클래스변수			클래스 블럭					컴파일될때 자동으로 생성되는 변수
 *    					class ClassName {		*** 한개만 생성되기때문에 모든 객체 공유하는 변수
 *    						******
 *    					}						class ClassName {
 *    												static int a;    											
 *    					=> 클래스 영역안에				static int b;
 *    												static String c, d, e;
 *    											}											
 *    						1) 인스턴스 변수
 *    						2) 클래스 변수
 ...
 *    										
 *    	-----------------------------------------------------
 *    	매개변수			메소드 괄호안에 선언된변수		메소드 호출후에 데이터를 전송했을 경우에 생성되는 변수
 ...
 *    	-----------------------------------------------------
 *    	지역변수			메소드 블럭안에 선언되는변수		메소드 호출시마다 생성되는 변수
 *    					메소드명() {				display() { 
 *    						****					int a = 0;
 *    					}						}
 *    											*** 루프변수 (반복문)
 ...
 *    -------------------->
 *    s1.schoolName
 *    Student.schoolName (클래스이름으로도 사용가능) => 메소드, 변수 클래스명으로 접근하면 (static) : Math
 *    
 *    String s = "Hello Java";
 *    s.substring()
 *    s.trim()
...
public class 변수종류_1 {
	...
	public static void main(String[] args) {
		...
		/*
		 *   인스턴스변수는 객체가 생성될때 객체마다 각기 다른 메모리를 생성하기 떄문에 다른 값을 유지 (더 많이쓰임)
