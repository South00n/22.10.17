 ...
 *    				  --- Acter
 *    	   ---------- 개발자 마다 데이터가 다르다
 *    	   1) 설계에 필요한 데이터를 설계 (데이터 : 변수)
 *    	   2) 메소드 (기능)
 *    	   3) 변수 -> 변수의 초기화, 저장 : 생성자 (클래스를 생성할떄 초기값을 넣어주는것)
 *    								 ----- 반드시 클래스명과 동일
 *    	   4) 메모리에 저장시에는
 ...
 *    		  ---------------------- 기본형과 동일하게 생각 (형변환)
 *    		  사용자 정의 데이터형 
 *    2. 데이터 관련 : 데이터를 보호 (캡슐화) -> 키워드를 이용해서 데이터를 은닉화
 *    								   ----- private
 *    3. 메소드 관련 : 기능 변경, 추가 (오버라이딩, 오버로딩) ==> 다형성
 *    4. 클래스 관련 : 있는 그대로 사용(더 많이 사용), 변경 후 사용
 *    				-----------	 ---------
 *    					포함 (has-a)	상속 (is-a) : 사용자 정의 (스프링은 독릭적인 클래스)
 *    					---------- 라이브러리
 *    ----------------- 유지보수에 초점을 맞춘 프로그램
 */
