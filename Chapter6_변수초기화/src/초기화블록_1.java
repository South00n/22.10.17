 ...
 *    			String s = "aaa"; ====== null
 *    		  }  
 *    		  에) Movie[] movie = new Movie[1938]; --> 초기화
 *    
 ...
 *    	 기본값 --> 명시적 초기화 --> 클래스 블록 --> 생성자 =======> 클래스 블록이 더 편함
 *    2. 인스턴스 (생성자가 호출돼야 한다)
 ...
 *    
 */
// => 중심 (변수) : 데이터형 클래스 (변수만 가지고있는)
/*
 * 	  클래스의 구성요소
 * 	  ------------
 * 		변수 : 데이터 저장
 * 			  공유 변수 =========> 모든 객체가 동일하게 사용 ==> static
 * 			  객체 변수 (인스턴스 변수) ===> new 할때마다 따로 생성하는 변수
...
// => 중심 (메소드) : 데이터를 관리 / 액션 클래스 (변수를 가지고있지 않고 기능위주)
/*
	...
	static {
		...
		try {
			...
			for (int i = 0; i < title.size(); i++) {
				movie[i] = new Movie();
				movie[i].tilte = title.get(i).text();
